...
function createLocation(rawLocation) {
  ...
  if (tc.not.isNullOrUndefined(rawLocation.replace) && rawLocation.replace === true) {
    location.action = HISTORY_ACTION.REPLACE;
  }

  if (tc.not.isNullOrUndefined(rawLocation.name) && tc.isString(rawLocation.name)) {
    location.name = rawLocation.name;
  }

  if (tc.not.isNullOrUndefined(rawLocation.hash) && tc.isString(rawLocation.hash)) {
    location.hash = rawLocation.hash.replace('#', '');
  } // Param object


  if (tc.not.isNullOrUndefined(rawLocation.params) && tc.isObject(rawLocation.params)) {
  ...
  if (tc.not.isNullOrUndefined(rawLocation.query) && tc.isObject(rawLocation.query)) {
...
function createRouteConfig(prefab) {
  ...
  if (tc.not.isNullOrUndefined(prefab.component) && tc.not.isFunction(prefab.component) && tc.not.isPromise(prefab.component)) {
  ...
  } else if (tc.not.isString(prefab.redirect) && tc.not.isObject(prefab.redirect) && tc.not.isFunction(prefab.redirect)) {
...
var Router = /*#__PURE__*/function () {
  ...
  function Router(opts) {
    ...
    this.onNavigationChanged(function () {
      setTimeout(function () {
        var _window$history$state, _window$history$state2;

        window.scrollTo({
          top: ((_window$history$state = window.history.state) === null || _window$history$state === void 0 ? void 0 : (_window$history$state2 = _window$history$state.scrollPosition) === null || _window$history$state2 === void 0 ? void 0 : _window$history$state2.y) || 0,
  ...
  _createClass(Router, [{
    ...
    value: function push(rawLocation, onComplete, onAbort) {
      ...
      this.resolveRoute(location, tc.isFunction(onComplete) ? onComplete : undefined, tc.isFunction(onAbort) ? onAbort : undefined);
    }
    /**
    ...
    value: function back() {
      ...
      this._history.goBack(); // TODO Здесь скролл вернуть в иходную позитцию?*
    ...
    value: function resolveRedirect(redirect, onComplete, onAbort) {
      ...
      this._pendingRoute = null;
      this.push(tc.isString(redirect) ? redirect : redirect, onComplete, onAbort);
    }
    /**
    ...
    value: function resolveNavigationGuard() {
      ...
      this._navigationGuards[index].guard(this._currentRoute, this._pendingRoute, function (next) {
        ...
        if (next == undefined) {
          _this6.resolveNavigationGuard(++index, onComplete, onAbort); // Cancel the route change
          ...
          _this6.push(next, onComplete, onAbort); // Unexpected next
    ...
    value: function finishNavigationChange(onComplete, onAbort) {
      ...
      var afterResolved = function afterResolved() {
        ...
        _this7._pendingRoute = null; // Resolve history update if needed
        ...
        if (historyFullURL(_this7._history.location) != _this7._currentRoute.fullPath) {
          ...
          if (_this7._currentRoute.action == HISTORY_ACTION.PUSH) {
            _this7._history.push(_this7._currentRoute.fullPath); // this._historyScroll.
            // Replace
...
export { HASH_TYPE, HISTORY_MODE as ROUTER_MODE, createRouter as default, router, trimPrefix, urlMatch, urlPrefix };
